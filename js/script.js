...
function elementIconHtml(element) {
  ...
  if (!id) return '';
  return `<img class="char-element" src="https://wuthering.wiki/img/element_${id}.png" alt="${element}" loading="lazy">`;
}
...
function charIconHtml(ch) {
  ...
  const img = url ? `<img class="char-icon-img" src="${url}" alt="${escapeHtml(ch.name)}" loading="lazy" onerror="this.style.display='none'">` : '';
